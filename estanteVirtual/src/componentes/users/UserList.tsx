...
const UserList: React.FC = () => {
    ...
    const handleDelete = async (id: string, nickname: string) => {
        ...
        if (currentUserProfile !== 'Administrador') { 
            alert("Apenas Administradores podem excluir usuários (RF4).");
            return;
        }
        if (window.confirm(`Confirma a exclusão do usuário "${nickname}"?`)) { 
            try {
                ...
                alert("Usuário excluído com sucesso (RF4).");
            } catch (err: any) {
    ...
    return (
        <div className="user-list">
            ...
            <div style={{ marginBottom: '20px' }}>
                <input 
                    placeholder="Buscar por Nome do Usuário (RF2)" 
                    onChange={(e) => setFilters({...filters, nome: e.target.value})} 
                    style={{ padding: '8px', marginRight: '10px' }}
                />
                <input 
                    placeholder="Buscar por Apelido (RF2)" 
